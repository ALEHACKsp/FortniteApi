...
const Lifetime = ({ stats }) => {
  ...
  // console.log("this is the playerstats", stats)
    ...
    .map((data, index) => {
     return (
         <SectionText key={index} >{data.key} : {data.value}</SectionText>
     )
    })
    ...
    .map((data, index)=> {
     return (
      <PlacementSectionText key={index} style={{color: 'black'}}>{data.key} : {data.value}</PlacementSectionText>
     )
    })
    ...
    return (
        ...
        <PlacementSection>{placementSection && placementSection.sort((a,b)=> {
          const splitA = a.props.children[0].match(/^\d+|\d+\b|\d+(?=\w)/g);
          const splitB = b.props.children[0].match(/^\d+|\d+\b|\d+(?=\w)/g);
